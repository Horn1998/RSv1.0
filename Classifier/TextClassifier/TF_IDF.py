...
from Common.FileProcess.BunchFile import BunchProcess
...
def segText(inputPath, resultPath):
    ...
    for eachDir in fatherLists:                                     # 遍历主目录中各个文件夹
        ...
        for eachFile in childLists:                                 # 遍历每个文件夹中的子文件
            ...
            content = TXTFile(eachPathFile).read()                  # 调用上面函数读取内容
            result = (str(content)).replace("\r\n", "").strip()     # 删除多余空行与空格
            cutResult = jieba.cut(result)                           # 默认方式分词，分词结果用空格隔开
            TXTFile(each_resultPath + eachFile).bulid(" ".join(cutResult))  # 调用上面函数保存文件
...
def bunchSave(inputFile, outputFile):
    ...
    Logger().get_log().error('finish bunch save + ', time.time(),  ', use time :' + str(start - time.time()))




def getTestSpace(testSetPath, trainSpacePath, stopWordList, testSpacePath):
    bunch = BunchProcess(testSetPath)
    # 构建测试集TF-IDF向量空间
    ...
    trainbunch = bunch.read(trainSpacePath)
    ...
    # 持久化
    bunch.bulid(testSpacePath, testSpace)
